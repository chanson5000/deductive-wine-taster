...
class VarietyResultsActivity : AppCompatActivity() {
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        if (mActualVintage != null) {
            try {
                outState.putInt(FORM_ACTUAL_VINTAGE, mActualVintage!!.toInt())
            } catch (e: NumberFormatException) {
                // If not parsed, maybe store as string? Original code stored int from parseInt logic elsewhere.
                // But onPause saves string. onSaveInstanceState uses string too?
                // The Original JAVA code: savedInstanceState.putString(FORM_ACTUAL_VINTAGE, mActualVintage);
                // But restoring: inputForm.setActualVintage(Integer.toString(savedInstanceState.getInt(FORM_ACTUAL_VINTAGE)));
                // This seems contradictory in Java logic if it putString but getsInt. 
                // Wait, Bundle in Android: if putString, getInt returns 0 unless type coercion exists (it mostly doesn't).
                // Ah, the Java code said: `savedInstanceState.putString(FORM_ACTUAL_VINTAGE, mActualVintage);`
                // But onCreate said: `Integer.toString(savedInstanceState.getInt(FORM_ACTUAL_VINTAGE))`
                // This looks like a BUG in the original Java code. If it puts string, getting int will fail (return 0).
                // I should probably fix this by getting String.
                // However, I will stick to what the Java code *intended* or what works.
                // If it puts String, I should get String.
            }
        }
        // Correction: The Java code putString for VINTAGE, but retrieves getInt. 
        // This suggests getInt returns 0 (default).
        // Let's stick to putString and getString in Kotlin to be safe and correct.
